...
public class MekanismKey extends AEKey {
    ...
    public boolean isTagged(TagKey<?> tag) {
        if (stack.getType() instanceof Gas gas) {
            return tag.registry().equals(MekanismAPI.gasRegistry()) && gas.is((TagKey<Gas>) tag);
        } else if (stack.getType() instanceof InfuseType infuse) {
            return tag.registry().equals(MekanismAPI.infuseTypeRegistry()) && infuse.is((TagKey<InfuseType>) tag);
        } else if (stack.getType() instanceof Pigment pigment) {
            return tag.registry().equals(MekanismAPI.pigmentRegistry()) && pigment.is((TagKey<Pigment>) tag);
        } else if (stack.getType() instanceof Slurry slurry) {
            return tag.registry().equals(MekanismAPI.slurryRegistry()) && slurry.is((TagKey<Slurry>) tag);
        } else {
            throw new UnsupportedOperationException();
        }
    }
